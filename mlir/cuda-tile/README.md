...
This example serves as a minimal demonstration of the Toy-CUDA compilation pipeline using the CUDA Tile dialect. By default, grid and block dimensions are set to `(1, 1, 1)`. As reflected in the generated CUDA Tile IR, no explicit grid or block configuration is emitted — the kernel executes with a single block and a single thread. While this configuration is not performance-optimized, it is sufficient to illustrate the end-to-end lowering through the CUDA Tile dialect.

> **Note:** On RTX 4090 with CUDA 12.x, custom grid dimensions are supported. However, you must provide a manually written CUDA kernel implementation in the `cuda_shim` directory that is compatible with the `sample/matmul.toy` example.

#### RTX 4090 with CUDA 12.x

The CUDA Tile dialect requires CUDA 13.x or later; kernels generated by this dialect are **not** compatible with earlier CUDA toolkit versions. To run the Toy-CUDA example on CUDA < 13.x, a manually written CUDA kernel must be provided as a substitute.

The recommended workflow is as follows:

1. **Compile the custom kernel to a cubin.**
   Use the source file `cuda_shim/outlined_gpu_kernel.cu` and compile it with `nvcc`, targeting the appropriate GPU architecture (e.g., `sm_89` for RTX 4090). This must be performed on a host with an NVIDIA CUDA 12.x toolchain installed. A Docker-based workflow is shown below:

   ```bash
   cd cuda_shim
   docker run -ti -v "$(pwd)":/workspace nvidia/cuda:12.4.1-devel-ubuntu22.04 bash
   # Inside the container:
   cd /workspace
   nvcc -std=c++17 -arch=sm_89 -cubin outlined_gpu_kernel.cu -o cuda_tile.cubin
   ```

2. **Place the cubin at the expected runtime path.**
   The runtime loader expects the binary at `/tmp/cuda_tile-94d280.bin`. Copy the compiled cubin to that location before executing the example:

   ```bash
   cp cuda_shim/cuda_tile.cubin /tmp/cuda_tile-94d280.bin
   ```

3. **Run the example with the `-use-cache` flag.**
   The `-use-cache` option instructs the runtime to load the pre-compiled binary instead of attempting on-the-fly compilation.

> **Warning:** If `-use-cache` is **not** specified, the runtime will delete the cached binary after execution. Ensure you have a backup copy before running without this flag.

> **Warning:** Because the custom kernel is hand-written to match the `sample/matmul.toy` example, the output may differ from a natively generated kernel.

#### RTX 5090 running with CUDA 13.x

- Show the outline gpu kernel IR

```bash
./build/Toy/toy-cuda sample/matmul.toy -emit=gpu-ir
...
```bash
./build/Toy/toy-cuda sample/matmul.toy -emit=cuda-tile-ir
# The GPU related actions will be used
# Grid dimensions: 1,1,1
# module {
#   toy.func @main() {
...
```bash
./build/Toy/toy-cuda sample/matmul.toy -emit=gpu-affine -opt
# The GPU related actions will be used
# Grid dimensions: 1,1,1
# module {
...
```bash
./build/Toy/toy-cuda sample/matmul.toy -emit=gpu-llvm -opt
# The GPU related actions will be used
# Grid dimensions: 1,1,1
# module {
...
```bash
./build/Toy/toy-cuda sample/matmul.toy -emit=nv-gpu-jit -opt
# The GPU related actions will be used
# Grid dimensions: 1,1,1
