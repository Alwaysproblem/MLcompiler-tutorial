...
register_pytree_node(
    dict, lambda d: map(tuple, unzip2(sorted(d.items()))),
...
def vmap_flat(f, in_axes, *args):
  axis_size, = {
      x.shape[ax]
      for x, ax in zip(args, in_axes) if ax is not not_mapped
  }
...
# %%
def _inline_literals(jaxpr: Jaxpr,
                     consts: list[Any]) -> tuple[Jaxpr, list[Any]]:
  ...
  new_eqns = [
      JaxprEqn(
          eqn.primitive, [literals.get(x, x)
                          for x in eqn.inputs], eqn.params, eqn.out_binders
      ) for eqn in jaxpr.eqns
  ]
...
def reduce_sum_abstract_eval(x: ShapedArray, *,
                             axis: tuple[int, ...]) -> list[ShapedArray]:
  axis_ = set(axis)
...
def pp_eqn(names: defaultdict[Var, str], eqn: JaxprEqn) -> PPrint:
  ...
  if rule:
    ...
    rhs = (
        pp(eqn.primitive.name) >> pp_params(eqn.params) >> pp(
            ' '.join(
...
# %%
jaxpr, consts, _ = make_jaxpr_v1(
    lambda x: 2. * x, raise_to_shaped(get_aval(3.))
)
print(jaxpr)
...
@lru_cache()
def xla_callable(
    hashable_jaxpr: IDHashable, hashable_consts: tuple[IDHashable, ...]
):
...
def _xla_params(c: xe.XlaBuilder,
                avals_in: list[ShapedArray]) -> list[xe.XlaOp]:
...
# %%
def jaxpr_subcomp(c: xe.XlaBuilder, jaxpr: Jaxpr,
                  args: list[xe.XlaOp]) -> list[xe.XlaOp]:
...
@lru_cache()
def vmap_jaxpr(jaxpr: Jaxpr, axis_size: int,
               bdims_in: tuple[BatchAxis, ...]) -> tuple[Jaxpr, list[Any]]:
...
def pprint_xla_call(names: defaultdict[Var, str], eqn: JaxprEqn) -> PPrint:
  ...
  rhs = (
      pp(eqn.primitive.name) >> pp_params(params_without_jaxpr) >> pp(
          ' '.join(
...
def tracers_to_jaxpr(
  ...
  tracer_to_var: dict[int, Var] = {
      id(t): Var(raise_to_shaped(t.aval))
      for t in tracers_in
  }
...
def recipe_to_eqn(
    tracer_to_var: dict[int, Var], recipe: JaxprEqnRecipe
) -> JaxprEqn:
...
def xla_call_partial_eval(trace, tracers, *, jaxpr, num_consts):
  ...
  eqn = JaxprEqnRecipe(
      xla_call_p, res_tracers + unknown_tracers,
      dict(jaxpr=jaxpr2, num_consts=0), [v.aval for v in jaxpr2.outs],
      map(ref, outs2)
  )
  for t in outs2:
    t.recipe = eqn
...
def xla_call_peval_eqn(
  ...
  eqn1 = JaxprEqn(
      xla_call_p, ins1, dict(jaxpr=jaxpr1, num_consts=0),
      out_binders1 + residuals
  )
  eqn2 = JaxprEqn(
      xla_call_p, residuals + ins2, dict(jaxpr=jaxpr2, num_consts=0),
      out_binders2
  )
...
def eval_jaxpr_transposed(jaxpr: Jaxpr, args: list[Any],
  ...
  return [
      read_cotangent(v) for v, x in zip(jaxpr.in_binders, args)
...
def mul_transpose_rule(cts, x, y):
  ...
  return [mul(z_bar, y), None
          ] if type(x) is UndefPrimal else [None, mul(x, z_bar)]
...
@lru_cache()
def transpose_jaxpr(jaxpr: Jaxpr,
                    undef_primals: tuple[bool, ...]) -> tuple[Jaxpr, list[Any]]:
...
def bind_cond(pred, *args, true_jaxpr, false_jaxpr):
  ...
  return bind(
      cond_p, pred, *args, true_jaxpr=true_jaxpr, false_jaxpr=false_jaxpr
  )
...
# %%
out, out_tan = jvp(
    lambda x: cond(True, lambda: x * x, lambda: 0.), (1., ), (1., )
)
print(out_tan)
...
def cond_translation(c, in_avals, in_vals, *, true_jaxpr, false_jaxpr):
  ...
  out = xops.Conditional(
      xops.ConvertElementType(pred, int_etype), [false_comp, true_comp],
      [operand] * 2
  )
...
def cond_partial_eval(trace, tracers, *, true_jaxpr, false_jaxpr):
  ...
  *jaxprs, out_uks, num_res = _cond_partial_eval(
      true_jaxpr, false_jaxpr, in_uks
  )
  ...
  known_vals = [t.pval.const for t in known_tracers]
  outs1_res = bind_cond(
      pred, *known_vals, true_jaxpr=t_jaxpr1, false_jaxpr=f_jaxpr1
  )
  ...
  eqn = JaxprEqnRecipe(
      cond_p, [pred_tracer_, *res_tracers, *unknown_tracers],
...
def _cond_partial_eval(
  ...
  t_jaxpr1, t_jaxpr2, _, t_nres = partial_eval_jaxpr(
      true_jaxpr, in_uks, out_uks
  )
  f_jaxpr1, f_jaxpr2, _, f_nres = partial_eval_jaxpr(
      false_jaxpr, in_uks, out_uks
  )
...
def _join_jaxpr_res(jaxpr1: Jaxpr, jaxpr2: Jaxpr, n1: int,
                    n2: int) -> tuple[Jaxpr, Jaxpr]:
  jaxpr1_type, jaxpr2_type = typecheck_jaxpr(jaxpr1), typecheck_jaxpr(jaxpr2)
...
def cond_peval_eqn(
  ...
  *jaxprs, unks_out, num_res = _cond_partial_eval(
      true_jaxpr, false_jaxpr, unks_in
  )
  t_jaxpr1, f_jaxpr1, t_jaxpr2, f_jaxpr2 = jaxprs
  ...
  eqn1 = JaxprEqn(
      cond_p, [eqn.inputs[0], *ins1],
      dict(true_jaxpr=t_jaxpr1, false_jaxpr=f_jaxpr1), outs1 + residuals
  )
  eqn2 = JaxprEqn(
      cond_p, [eqn.inputs[0], *residuals, *ins2],
      dict(true_jaxpr=t_jaxpr2, false_jaxpr=f_jaxpr2), outs2
  )
...
def cond_transpose_rule(cts, pred, *invals, true_jaxpr, false_jaxpr):
  ...
  outs = iter(outs)
  return [None
          ] + [next(outs) if type(x) is UndefPrimal else None for x in invals]
...
def pprint_cond(names: defaultdict[Var, str], eqn: JaxprEqn) -> PPrint:
  ...
  lhs = pp(' '.join(var_str(names, v) for v in eqn.out_binders))
  rhs = (
      pp(eqn.primitive.name) >> pp_params(new_params) >> pp(
          ' '.join(
              names[x] if isinstance(x, Var) else str(x.val) for x in eqn.inputs
          )
      )
